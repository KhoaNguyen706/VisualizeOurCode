 ...
 * the comments say why where it is not obvious.
 */
function leadTechnique(code: string, recursive: boolean): VisualizationTechnique {
  ...
  if (/\bmemo\b|\bdp\s*\[|\bdp\[|tabulation|bottom.?up|lru_cache|@cache\b/.test(c)) return "dp_grid";
 ...
 * indices on one array, and backtracking is a DFS is a recursion.
 */
type Family = "pointer" | "recursion" | "list";
...
const SUPPORTING: Array<[VisualizationTechnique, (c: string, recursive: boolean) => boolean]> = [
  ...
  ["dp_grid", (c) => /\bmemo\b|\bdp\s*\[|\bdp\[|lru_cache|@cache\b|tabulation|bottom.?up/.test(c)],
